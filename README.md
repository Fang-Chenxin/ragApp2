...
3. **语义表驱动的业务知识集中管理**  
   项目通过 `search_semantics` 和 `attribute_ontology` 集中管理商品同义词、品类别名、fallback 关系、品牌型号、场景标签和属性标准化等导购业务知识，避免业务规则散落在代码中。LLM 负责语义规划，语义表提供稳定业务边界，代码负责执行和校验。

---

## 对话处理流程

一轮导购请求的主要链路如下（按阶段顺序展示，实际存在大量并行重叠）：
...
| 阶段 | 主要任务 | 说明 |
| --- | --- | --- |
| 并行启动 | 需求分析、SearchPlan、首轮工具规划 | 三个任务同时启动，互不依赖 |
| RAG 处理 | 向量检索、LLM rerank | 紧跟并行启动阶段之后立即启动，与前三个任务并行执行 |
| 商品召回 | SQLite 直查、工具调用 | SearchPlan 完成后执行，工具调用支持并发 |
| **合并去重** | **RAG候选 + 工具候选 + 直查候选** | **按优先级顺序合并，保留首次出现的商品** |
| 约束过滤 | 数据库回查、约束校验、排序 | 确保最终推荐只基于真实商品 |
| 最终回复 | LLM 生成受约束回复 | 检查是否包含非白名单商品 |

最终回复阶段会检查模型是否提到非白名单商品。如果直接回复包含清单外商品，后端会强制进入受约束最终回复，降低商品幻觉风险。

### 并行处理机制

导购链路不是完全串行执行，后端采用"流水线式并行"策略，核心特点如下：

**并行启动阶段**：后端在 `parallel_bootstrap` 阶段同时启动三条 LLM 分支，**启动后立即进入下一阶段，不等待完成**：
- 需求分析 LLM：生成 Android 端可折叠的"已思考"内容
- SearchPlan 结构化规划：解析目标商品、品类、排除条件、价格偏好和对比意图
- 首轮工具规划：提前让 LLM 准备 `query_products` 工具调用

**RAG 检索与并行任务同时进行**：`parallel_bootstrap` 阶段结束后，立即进入 `rag_pipeline` 阶段启动向量检索，因此以下四个任务**几乎是同时并行执行的**：
- 需求分析 LLM
- SearchPlan 结构化规划
- 首轮工具规划
- RAG 向量检索

**流水线式等待**：在等待向量检索和 LLM rerank 完成的期间，会穿插输出已经生成的需求分析增量，避免前端空等。

**后续串行阶段**：SearchPlan 完成后才执行 SQLite 直查；工具调用阶段支持同一轮多个 `query_products` 并发执行。

因此日志中的分析、RAG、LLM、工具调用等阶段耗时不能简单相加等于总耗时，因为这些任务存在重叠。后端会记录 `parallel_enabled`、`vector_search`、`rag_rerank`、`llm_calls`、`tool_calls` 和 `total` 等字段，用于观察并行效果。客户端断开或流程提前结束时，后台未完成任务会被统一取消，避免资源浪费。

---

## 语义表与同义词策略

项目将导购业务知识集中维护在 `ecommerce_agent_dataset/search_semantics/` 目录和 `ecommerce_agent_dataset/attribute_ontology.json` 中，避免规则散落在代码里。整体原则是：**LLM 负责语义规划，语义表提供稳定业务边界，代码负责执行和校验**。
...
### 商品推荐能力
- 目标商品白名单：最终推荐只基于校验后的目标商品，降低商品幻觉风险。
...
### 异常处理
- 后端不可达、模型未配置、工具参数错误、RAG 超时和流式中断均有降级或提示。
