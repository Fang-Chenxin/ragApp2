...
✨ **阶段化工具聊天服务**: 导购主链路已按流式 Pipeline、RAG、prompt、目标商品选择和 trace 格式化拆入职责型模块
