...
graph TB
    ...
    subgraph "Backend API"
        ...
        PSAPI["sqlite_product_search.py<br/>商品查询接口"]
    end
...
│   │   └── sqlite_product_search.py # SQLite 商品查询接口
│   ├── config/                # 配置管理
│   │   └── settings.py        # 环境变量配置
│   ├── service/               # 业务逻辑层
...
**功能描述**: 集成 SQLite 商品数据库查询能力，支持 LLM 通过 `query_products` 工具自动调用

**实现要点**:
- 真实查询逻辑位于 `backend/service/product_search/sqlite_search.py`
- OpenAI 工具定义与分发位于 `backend/service/product_search/query_tool.py`
- 商品搜索规则解析位于 `backend/service/product_search/engine.py`
- 商品查询既可由 `/api/product-search/*` 直接访问，也可被 `/api/chat` 的导购链路间接调用
- 工具返回的是数据库真实结果，不再依赖旧版 `EcommerceService` 命名
...
| `/api/product-search/health` | GET | 服务健康检查 |
...
**功能描述**: 在 SearchPlan 中识别用户是“随便看看/探索了解”还是“明确购买/需要推荐”，并将识别结果用于最终导购回复口径和调试日志。

**实现要点**:
- SearchPlan 新增 `purchase_intent` 和 `purchase_intent_reason`
- `browsing` 用于“随便看看、看看有没有、了解一下、有什么可选”等探索表达
- `purchase_ready` 用于“想买、我要、需要、预算、推荐一款、急用”等明确导购表达
- 旧模型未输出字段时按用户原话规则兜底，非法值统一按 `purchase_ready` 处理
- 最终回复阶段根据购买阶段调整口径，但不改变商品召回数量和目标商品白名单策略

**流程收益**:
- 探索型用户收到更自然的“先看方向、对比参考”式回复
- 明确购买型用户继续收到有优先级的推荐与取舍建议
- `backend.log` 可直接展示购买意图和判断依据，便于验收和排查

### 3.13 多轮上下文、排除语义与对比决策

**功能描述**: 导购 Agent 支持在多轮对话中承接上一轮商品和筛选条件，正确处理排除/否定语义，并在多商品取舍场景中生成结构化对比结果。

**实现要点**:
- SearchPlan 新增 `is_followup`、`context_carryover`、`excluded_brands`、`excluded_terms`、`excluded_attributes`、`comparison_intent`、`comparison_dimensions` 和 `price_preference`
- 目标商品白名单过滤统一应用品牌、关键词和属性排除，避免被排除商品重新进入最终推荐
- 对比场景下最终回复强制走受约束生成，优先输出标准 Markdown 对比表
- Android 端将复杂 Markdown 表格解析为“推荐排序 + 商品卡片矩阵”，适配手机屏幕下的长文本、多维度对比阅读

**流程收益**:
- “再便宜点的呢？”可承接上一轮品类并优先低价候选
- “除了耐克”“不要含酒精”不会把排除项当成正向检索条件
- “哪一种更划算”“哪一个适合老人使用”可触发对比决策
- 多维度对比在 Android 端避免表格文字重叠、内容溢出和过度横向滑动
...
| 变更文档 | 类型 | 影响范围 | 完成时间 |
|---------|------|---------|---------|
| [多对话页面](./变更摘要/多对话页面.md) | 新功能 | 🔴 高 | 2026-05-24 |
| [思考过程持久化与显示优化](./变更摘要/思考过程持久化与显示优化.md) | Bug修复 | 🟡 中 | 2026-05-24 |
| [后端地址可配置化](./变更摘要/后端地址可配置化.md) | 功能改进 | 🟡 中 | 2026-05-24 |
| [电商数据库服务](./变更摘要/电商数据库服务.md) | 新功能 | 🟡 中 | 2026-05-24 |
| [查询工具接入后端](./变更摘要/查询工具接入后端.md) | 架构重构 | 🔴 高 | 2026-05-24 |
| [对话列表点击当前空对话误删除修复](./变更摘要/对话列表点击当前空对话误删除修复.md) | Bug修复 | 🟢 低 | 2026-05-24 |
| [变更摘要文档规范更新](./变更摘要/变更摘要文档规范更新.md) | 配置/工具 | 🟢 低 | 2026-05-28 |
| [导购聊天服务与流式交互优化](./变更摘要/导购聊天服务与流式交互优化.md) | 新功能 | 🟡 中 | 2026-05-28 |
| [后端商品搜索服务优化](./变更摘要/后端商品搜索服务优化.md) | 功能改进 | 🟡 中 | 2026-05-29 |
| [需求分析折叠展示与思考流程统一](./变更摘要/需求分析折叠展示与思考流程统一.md) | 架构重构 | 🔴 高 | 2026-05-29 |
| [导购流程并行化与目标商品白名单](./变更摘要/导购流程并行化与目标商品白名单.md) | 架构重构 | 🟡 中 | 2026-06-03 |
| [Android文本选取与会话切换稳定性修复](./变更摘要/Android文本选取与会话切换稳定性修复.md) | Bug修复 | 🟡 中 | 2026-06-01 |
| [模型选择与自定义模型管理](./变更摘要/模型选择与自定义模型管理.md) | 新功能 | 🔴 高 | 2026-06-02 |
| [后端服务优化与Android端稳定性改进](./变更摘要/后端服务优化与Android端稳定性改进.md) | 功能优化 | 🟡 中 | 2026-06-03 |
| [后端稳定性与测试同步优化](./变更摘要/后端稳定性与测试同步优化.md) | 稳定性修复 | 🟡 中 | 2026-06-07 |
| [外部向量化模型接入与状态展示](./变更摘要/外部向量化模型接入与状态展示.md) | 功能改进 | 🟡 中 | 2026-06-08 |
| [LLM搜索计划与商品匹配优化](./变更摘要/LLM搜索计划与商品匹配优化.md) | 功能改进 | 🟡 中 | 2026-06-08 |
| [语义表驱动的商品搜索业务知识注入](./变更摘要/语义表驱动的商品搜索业务知识注入.md) | 功能改进 | 🟡 中 | 2026-06-08 |
| [导购购买意图识别与回复口径优化](./变更摘要/导购购买意图识别与回复口径优化.md) | 功能改进 | 🟡 中 | 2026-06-09 |
| [导购上下文排除与对比决策增强](./变更摘要/导购上下文排除与对比决策增强.md) | 功能改进 | 🟡 中 | 2026-06-09 |
...
✨ **多轮导购决策增强**: SearchPlan 支持追问承接、排除语义、价格偏好和多商品对比意图，Android 端以商品卡片矩阵呈现复杂对比结果
